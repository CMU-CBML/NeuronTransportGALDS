	...
	for (e=0;e<bzmesh_process.size();e++){
		...
		for (int i = 0; i < nen; i++){
			...
			if (cpts[A].label == 2) {
				//anyoutlet
				p_bc = 0.0;
				ApplyBoundaryCondition(p_bc, i, 3, Ke, Re);
			}
		}
...
void NS_3Dsteady::Run(const vector<Vertex3D>& cpts, const vector<Element3D>& tmesh, const vector<array<double, 3>>& velocity_bc, string fn)
{
	...
	for (l = 0; l<n_iterator; l++){
		...
		// ! Save GR and GK into txt file for visualization
		// PetscViewer viewer1;
		// PetscViewer viewer2;
		// PetscViewerASCIIOpen(PETSC_COMM_WORLD, "matrix_output_GR.txt", &viewer1);
		// VecView(GR, viewer1);
		// PetscViewerDestroy(&viewer1);
		// PetscViewerASCIIOpen(PETSC_COMM_WORLD, "matrix_output_GK.txt", &viewer2);
		// MatView(GK, viewer2);
		// PetscViewerDestroy(&viewer2);
		// MPI_Barrier(comm);

		PetscPrintf(PETSC_COMM_WORLD, "Solving...\n");
		// ! Check the approximate condition number of GK
		PetscReal norm_fro, norm_inv;
		MatNorm(GK, NORM_FROBENIUS, &norm_fro);      // Frobenius 范数
		MatNorm(GK, NORM_1, &norm_inv);            // 1-范数（需近似逆矩阵）
		PetscPrintf(PETSC_COMM_WORLD, "Approx condition numer: %g\n", norm_fro * norm_inv);

		// ! Matshift to avoid singularity for GK (not helping much)
		// MatShift(GK, 1e-8);  // 添加正则化项
		// MatAssemblyBegin(GK, MAT_FINAL_ASSEMBLY);  // 重新组装（可选，但建议）
		// MatAssemblyEnd(GK, MAT_FINAL_ASSEMBLY);
		...
		// ! Following are the code that worked for NMO_66731
		/*Petsc KSP solver setting*/
		// KSPCreate(PETSC_COMM_WORLD, &ksp);
		// KSPReset(ksp);
		// KSPSetOperators(ksp, GK, GK);		
		...
		// PC subpc;
		// PetscInt first,nlocal;
		// KSPSetTolerances(ksp, 1.e-12, PETSC_DEFAULT, PETSC_DEFAULT, 100000);
		// KSPSetPCSide(ksp, PC_RIGHT);
		// KSPSetFromOptions(ksp);
		// KSPSetUp(ksp);
		// PCBJacobiGetSubKSP(pc, &nlocal, &first, &subksp);
		...
		// 	PCSetType(subpc, PCILU);
		// 	KSPSetType(subksp[i], KSPGMRES);
		// 	KSPSetInitialGuessNonzero(subksp[i], PETSC_FALSE);
		// 	KSPSetTolerances(subksp[i], 1e-3, 1e-10, PETSC_DEFAULT, 30);
		// 	PCFactorSetLevels(subpc, 1);  // ILU(1)
		// 	PCFactorSetShiftType(subpc, MAT_SHIFT_NONZERO);  // 避免零主元
		// 	PCFactorSetShiftAmount(subpc, PETSC_DECIDE);  // 自動計算shift量
		// 	KSPSetPCSide(subksp[i], PC_RIGHT);
		// }

		// ! Check norm of GR and GK. Check if nan or inf in GR and GK.
		PetscReal gr_norm, mat_norm;
		VecNorm(GR, NORM_INFINITY, &gr_norm);
		MatNorm(GK, NORM_FROBENIUS, &mat_norm);
		PetscPrintf(PETSC_COMM_WORLD, "GR Norm: %g\n", gr_norm);
		PetscPrintf(PETSC_COMM_WORLD, "GK Norm: %g\n", mat_norm);
		PetscReal cond;
		if (PetscIsInfOrNanReal(gr_norm) || PetscIsInfOrNanReal(mat_norm)) {
			PetscPrintf(PETSC_COMM_WORLD, "There are inf or nan in GR and GK\n");
		}
		...
		//! Check if there is nan or inf in the solution
		PetscReal norm;
		VecNorm(temp_solution, NORM_INFINITY, &norm);
		if (PetscIsInfOrNanReal(norm)) {
			PetscPrintf(PETSC_COMM_WORLD, "Solver result contains inf\n");
			VecView(temp_solution, PETSC_VIEWER_STDOUT_WORLD);
		}
