...
void Transport::InitializeProblem(const int n_bz, vector<array<double, 3>> &velocity_node, const vector<double>& N0_ini, const vector<double>& Nplus_ini, const vector<double>& Nminus_ini, const vector<double>& var)
{
	...
	if (var.size() != 0){
		...
		for (i=0; i<par.size(); i++)
			PetscPrintf(PETSC_COMM_WORLD, "par[%d] = %f\n", i, par[i]);
	}
	else
		cerr << "0 variables!\n";
	dt = var[7];
...
// 											+ dt*par[1] * par[12] *Npx[i] * (dNdx[i][0] * dNdx[j][0] + dNdx[i][1] * dNdx[j][1] + dNdx[i][2] * dNdx[j][2])*detJ;
// 		}
// 	}
// }
// ! Tangent is new
void Transport::Tangent(const int nen, vector<double>& Nx, vector<double>& Npx, vector<double>& Nmx, vector<array<double, 3>>& dNdx, double vp[3], double vm[3], double detJ, double k3_val, double k4_val, double k5_val, vector<vector<double>>& EMatrixSolve)
{
    /*calculate tangent matrix*/
    int i, j, A, B;
    for (i = 0; i < nen; i++){
        for (j = 0; j < nen; j++){
            EMatrixSolve[i + 0][j + 0] += ((1+ dt*(k3_val + k4_val))*Nx[i]*Nx[j] + dt*par[0]*(dNdx[i][0] * dNdx[j][0] + dNdx[i][1] * dNdx[j][1] + dNdx[i][2] * dNdx[j][2]))*detJ;
            EMatrixSolve[i + 0][j + nen] += -k5_val * dt *Nx[i] * Nx[j] * detJ; // Use k5_val
            EMatrixSolve[i + nen][j + 0] += -k3_val * dt *Npx[i] * Nx[j] * detJ; // Use k3_val
            // EMatrixSolve[i + nen][j + nen] += (1+ dt*par[5])*Npx[i] * Nx[j] * detJ + dt*Npx[i] * (vp[0] * dNdx[j][0] + vp[1] * dNdx[j][1] + vp[2] * dNdx[j][2])*detJ;
            EMatrixSolve[i + nen][j + nen] += (1+ dt*k5_val)*Npx[i] * Nx[j] * detJ + dt*Npx[i] * (vp[0] * dNdx[j][0] + vp[1] * dNdx[j][1] + vp[2] * dNdx[j][2])*detJ \
                                            + dt*par[1] * par[12] *Npx[i] * (dNdx[i][0] * dNdx[j][0] + dNdx[i][1] * dNdx[j][1] + dNdx[i][2] * dNdx[j][2])*detJ;
        }
    }
}
...
// ! This is new
void Transport::BuildLinearSystemProcess(const vector<Element3D>& tmesh, const vector<Vertex3D> &cpts, const vector<array<double, 3>> velocity_node)
{
        ...
        for (int i = 0; i<Gpt.size(); i++){
            for (int j = 0; j<Gpt.size(); j++){
                for (int k = 0; k < Gpt.size(); k++){
                    ...
                    // --- NEW: Get interpolated K values at the Gauss point ---
                    double k3_at_gp, k4_at_gp, k5_at_gp;
                    GetInterpolatedKValues(Nx, bzmesh_process[e].IEN, k3_at_gp, k4_at_gp, k5_at_gp);
                    // --- END NEW ---
                    ...
                    // Note: If par[5] and par[6] in SUPGcoefficient are meant to be k5_val,
                    // you might need to pass k5_at_gp to it as well if `s` is a coefficient.
                    // For now, assuming `s` is a generic stability factor.
                    SUPGcoefficient(par[5], vplus_tmp, dudx, dNdx, tau[0]); // par[5] might need to be k5_at_gp
                    SUPGcoefficient(par[6], vminus_tmp, dudx, dNdx, tau[1]); // par[6] might need to be k6_at_gp

                    WeightingFunction(vplus_tmp, par[5], tau[0], Nx, dNdx, Npx); // par[5] might need to be k5_at_gp
                    WeightingFunction(vminus_tmp, par[6], tau[1], Nx, dNdx, Nmx); // par[6] might need to be k6_at_gp

                    detJ = wght[i] * wght[j] * wght[k] * detJ;

                    // --- Update Tangent call with interpolated K values ---
                    Tangent(nen, Nx, Npx, Nmx, dNdx, vplus_tmp, vminus_tmp, detJ, k3_at_gp, k4_at_gp, k5_at_gp, EMatrixSolve);
                    // --- END UPDATE ---
...
void Transport::VisualizeVTK_ControlMesh(const vector<Vertex3D>& spt, const vector<Element3D>& mesh, int step, string fn)
{
	...
	if (fout.is_open())
	{
		...
		fout << "SCALARS N_plus double 1\nLOOKUP_TABLE default\n";
		for (uint i = 0; i<N_plus.size(); i++)
		{
			fout << N_plus[i] << "\n";
		}
		fout << "SCALARS N_minus double 1\nLOOKUP_TABLE default\n";
		for (uint i = 0; i<N_plus.size(); i++)
		{
			fout << N_minus[i] << "\n";
		}
...
// ! This is new
void Transport::Run(const vector<Vertex3D>& cpts, const vector<array<double, 3>> velocity_node, const vector<Element3D> &tmesh, string path_in, string path_out, string k_vtk_file)
{
    PetscPrintf(PETSC_COMM_WORLD, "Diffusion...\n");
	PetscPrintf(PETSC_COMM_WORLD, "Process %d: nstep = %d\n", comRank, nstep);
    ReadBezierElementProcess(path_in);

    // --- NEW: Read K values from VTK file at the start ---
    ReadKValuesFromVTK(k_vtk_file, cpts);
	PetscPrintf(PETSC_COMM_WORLD, "K values read from VTK file.\n");
    // --- END NEW ---
    ...
    for (int n = 0; n < nstep; ++n)
    {
        ...
        VecSet(GR, 0);
        t0 = time(NULL);
        // --- Update BuildLinearSystemProcess call ---
        BuildLinearSystemProcess(tmesh, cpts, velocity_node); // Removed Vplus, Vminus args
        // --- END UPDATE ---
        VecAssemblyEnd(GR);
        ...
        PetscErrorCode ierr; // For PETSc error checking
        if (judge == 0)	{
...
void Transport::ReadKValuesFromVTK(string k_vtk_file, const vector<Vertex3D>& cpts)
{
    // This is a simplified VTK reader for UNSTRUCTURED_GRID with POINT_DATA.
    // It assumes the order of points in VTK matches your cpts order.
    // For robust VTK parsing, consider a dedicated VTK library (e.g., VTK itself, or custom parser).

    ifstream fin(k_vtk_file);
    string line;
    int num_points = 0;
    bool reading_points = false;
    bool reading_data = false;
    string current_scalar_name;

    K3_node_values.resize(cpts.size());
    K4_node_values.resize(cpts.size());
    K5_node_values.resize(cpts.size());

    if (!fin.is_open()) {
        PetscPrintf(PETSC_COMM_WORLD, "Error: Cannot open K-value VTK file %s!\n", k_vtk_file.c_str());
        MPI_Abort(comm, 1); // Abort if file cannot be opened
        return;
    }

    PetscPrintf(PETSC_COMM_WORLD, "Reading K-values from VTK file: %s\n", k_vtk_file.c_str());
    ...
    while (getline(fin, line)) {
        ...
        if (keyword == "POINTS") {
            ss >> num_points;
            if (num_points != cpts.size()) {
                PetscPrintf(PETSC_COMM_WORLD, "Warning: Number of points in VTK file (%d) does not match control points (%zu).\n", num_points, cpts.size());
                // This might lead to issues if point order or count doesn't match
            }
            // Skip actual point coordinates, as we only need the data associated with them
            reading_points = true;
            reading_data = false;
        } else if (keyword == "CELLS" || keyword == "CELL_TYPES") {
            reading_points = false; // Stop reading points, prepare for next section
            reading_data = false;
        } else if (keyword == "POINT_DATA") {
            ss >> num_points; // Number of points for data
            reading_points = false; // Ensure we are not in points section
            reading_data = true; // Start looking for SCALARS
        } else if (keyword == "SCALARS" && reading_data) {
            ss >> current_scalar_name;
            string type_str, count_str;
            ss >> type_str >> count_str; // e.g., "double 1"
            PetscPrintf(PETSC_COMM_WORLD, "Found scalar field: %s\n", current_scalar_name.c_str());

            // Read the LOOKUP_TABLE line
            getline(fin, line);
            if (line.find("LOOKUP_TABLE") == string::npos) {
                 PetscPrintf(PETSC_COMM_WORLD, "Warning: Expected LOOKUP_TABLE after SCALARS, but got: %s\n", line.c_str());
                 // This might break if VTK format varies slightly
            }
            ...
            for (int i = 0; i < num_points; ++i) {
                ...
                if (current_scalar_name == "K3_field") { // Match your scalar field names
                    K3_node_values[i] = value;
                } else if (current_scalar_name == "K4_field") {
                    K4_node_values[i] = value;
                } else if (current_scalar_name == "K5_field") {
                    K5_node_values[i] = value;
                }
            }
            // Reset for next scalar field
            current_scalar_name = "";
        }
    }
    fin.close();
    PetscPrintf(PETSC_COMM_WORLD, "Finished reading K-values from VTK file.\n");
}

void Transport::GetInterpolatedKValues(const vector<double>& Nx, const vector<int>& IEN, double& k3_val, double& k4_val, double& k5_val)
{
    k3_val = 0.0;
    k4_val = 0.0;
    k5_val = 0.0;

    // Nx is the shape function values at the current Gauss point for the current element's basis functions
    // IEN provides the global indices of the nodes for the current element
    // Kx_node_values stores the K values at *all global nodes*

    for (size_t i = 0; i < Nx.size(); ++i) { // Loop over local basis functions (nodes of the element)
        int global_node_idx = IEN[i]; // Get the global index of the i-th node of this element

        // Interpolate K values using the shape functions
        // This assumes Kx_node_values are indexed by global node ID
        k3_val += Nx[i] * K3_node_values[global_node_idx];
        k4_val += Nx[i] * K4_node_values[global_node_idx];
        k5_val += Nx[i] * K5_node_values[global_node_idx];
    }
}